...
namespace LocalNote.ViewModels
{
    public class MainPageData : INotifyPropertyChanged
    {
        ...
        private async Task RefreshNoteList()
        {
        ...
        private async Task<Note> CreateNewNote(string title, string content)
        {
            int newNoteId = 1;
        ...
        private async Task WriteToNote(Note note, string content)
        {
        ...
        private async Task TitleDialog()
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                {
                    MessageDialog duplicateNameDialog = new MessageDialog("That title is already taken. Please try a different title.");
